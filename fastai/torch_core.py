...
# %% ../nbs/00_torch_core.ipynb #a5f202c7
from .imports import *
from .torch_imports import *
from packaging.version import parse

# %% ../nbs/00_torch_core.ipynb #f69002ba
_all_ = ['progress_bar','master_bar']

# %% ../nbs/00_torch_core.ipynb #a6fdfe9c
defaults.benchmark = True

# %% ../nbs/00_torch_core.ipynb #940ae64f
...
# %% ../nbs/00_torch_core.ipynb #9b4025c2
...
# %% ../nbs/00_torch_core.ipynb #c82efac4
def _fig_bounds(x):
    r = x//32
    return min(5, max(1,r))

# %% ../nbs/00_torch_core.ipynb #c008ee0a
...
# %% ../nbs/00_torch_core.ipynb #1990597b
@delegates(show_image, keep=True)
def show_titled_image(o, **kwargs):
    "Call `show_image` destructuring `o` to `(img,title)`"
    show_image(o[0], title=str(o[1]), **kwargs)

# %% ../nbs/00_torch_core.ipynb #bac10ef6
@delegates(subplots)
...
# %% ../nbs/00_torch_core.ipynb #4e191057
...
# %% ../nbs/00_torch_core.ipynb #0c1a1b3d
...
# %% ../nbs/00_torch_core.ipynb #205fe1ce
...
# %% ../nbs/00_torch_core.ipynb #cb26edee
...
# %% ../nbs/00_torch_core.ipynb #f70da221
@contextmanager
...
# %% ../nbs/00_torch_core.ipynb #93c1199f
def unsqueeze(x, dim=-1, n=1):
    "Same as `torch.unsqueeze` but can add `n` dims"
    for _ in range(n): x = x.unsqueeze(dim)
    return x

# %% ../nbs/00_torch_core.ipynb #2ce5e243
def unsqueeze_(x, dim=-1, n=1):
    "Same as `torch.unsqueeze_` but can add `n` dims"
    for _ in range(n): x.unsqueeze_(dim)
    return x

# %% ../nbs/00_torch_core.ipynb #53095ccd
def _fa_rebuild_tensor (cls, *args, **kwargs): return cls(torch._utils._rebuild_tensor_v2(*args, **kwargs))
def _fa_rebuild_qtensor(cls, *args, **kwargs): return cls(torch._utils._rebuild_qtensor  (*args, **kwargs))

# %% ../nbs/00_torch_core.ipynb #12bb81b5
...
# %% ../nbs/00_torch_core.ipynb #da141171
...
# %% ../nbs/00_torch_core.ipynb #27ef4533
...
# %% ../nbs/00_torch_core.ipynb #0e8cccaa
def to_cpu(b):
    "Recursively map tensors in `b ` to the cpu."
    return to_device(b,'cpu')

# %% ../nbs/00_torch_core.ipynb #3237faea
def to_np(x):
    "Convert a tensor to a numpy array."
    return apply(lambda o: o.data.cpu().numpy(), x)

# %% ../nbs/00_torch_core.ipynb #59e65b25
...
# %% ../nbs/00_torch_core.ipynb #cd78c077
...
# %% ../nbs/00_torch_core.ipynb #df2dff0c
@patch
...
# %% ../nbs/00_torch_core.ipynb #27b55d9b
...
# %% ../nbs/00_torch_core.ipynb #63b3ba7c
class TensorImageBase(TensorBase):
    _show_args = ArrayImageBase._show_args
    def show(self, ctx=None, **kwargs):
        return show_image(self, ctx=ctx, **{**self._show_args, **kwargs})

# %% ../nbs/00_torch_core.ipynb #34dbe862
class TensorImage(TensorImageBase): pass

# %% ../nbs/00_torch_core.ipynb #57d23eb0
class TensorImageBW(TensorImage): _show_args = ArrayImageBW._show_args

# %% ../nbs/00_torch_core.ipynb #b5907b47
...
# %% ../nbs/00_torch_core.ipynb #ea61a8a8
...
# %% ../nbs/00_torch_core.ipynb #cc100ce7
def concat(*ls):
...
# %% ../nbs/00_torch_core.ipynb #606b275b
class Chunks:
...
# %% ../nbs/00_torch_core.ipynb #df7d1094
...
# %% ../nbs/00_torch_core.ipynb #ba1d47c7
class ShowTitle:
...
# %% ../nbs/00_torch_core.ipynb #bb564368
@patch
...
# %% ../nbs/00_torch_core.ipynb #587159d7
def get_empty_df(n):
    "Return `n` empty rows of a dataframe"
    df = pd.DataFrame(index = range(n))
    return [df.iloc[i] for i in range(n)]

# %% ../nbs/00_torch_core.ipynb #f7b0b535
def display_df(df):
...
# %% ../nbs/00_torch_core.ipynb #ef365427
def get_first(c):
    "Get the first element of c, even if c is a dataframe"
    return getattr(c, 'iloc', c)[0]

# %% ../nbs/00_torch_core.ipynb #850e0828
def one_param(m):
    "First parameter in `m`"
    return first(m.parameters())

# %% ../nbs/00_torch_core.ipynb #6ad3b55e
...
# %% ../nbs/00_torch_core.ipynb #baf2e5e6
def find_device(b):
    "Recursively search the device of `b`."
    return item_find(b).device

# %% ../nbs/00_torch_core.ipynb #41af3203
def find_bs(b):
...
# %% ../nbs/00_torch_core.ipynb #b99feeff
def np_func(f):
...
# %% ../nbs/00_torch_core.ipynb #77a539b1
class Module(nn.Module, metaclass=PrePostInitMeta):
    "Same as `nn.Module`, but no need for subclasses to call `super().__init__`"
    def __pre_init__(self, *args, **kwargs): super().__init__()
    def __init__(self): pass

# %% ../nbs/00_torch_core.ipynb #256eb43a
from torch.nn.parallel import DistributedDataParallel

# %% ../nbs/00_torch_core.ipynb #05f62aa4
def get_model(model):
    "Return the model maybe wrapped inside `model`."
    return model.module if isinstance(model, (DistributedDataParallel, nn.DataParallel)) else model

# %% ../nbs/00_torch_core.ipynb #283f07bd
def one_hot(x, c):
...
# %% ../nbs/00_torch_core.ipynb #05c54494
def one_hot_decode(x, vocab=None):
    return L(vocab[i] if vocab else i for i,x_ in enumerate(x) if x_==1)

# %% ../nbs/00_torch_core.ipynb #373c50a2
def params(m):
    "Return all parameters of `m`"
    return [p for p in m.parameters()]

# %% ../nbs/00_torch_core.ipynb #018cb134
def trainable_params(m):
    "Return all trainable parameters of `m`"
    return [p for p in m.parameters() if p.requires_grad]

# %% ../nbs/00_torch_core.ipynb #641ba2b1
norm_types = (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.InstanceNorm1d, nn.InstanceNorm2d, nn.InstanceNorm3d, nn.LayerNorm)

# %% ../nbs/00_torch_core.ipynb #5fd95264
...
# %% ../nbs/00_torch_core.ipynb #b00b6695
...
# %% ../nbs/00_torch_core.ipynb #1bcdba1a
@patch
...
# %% ../nbs/00_torch_core.ipynb #21206c11
@patch
...
# %% ../nbs/00_torch_core.ipynb #4d7a59ad
def logit(x):
    "Logit of `x`, clamped to avoid inf."
    x = x.clamp(1e-7, 1-1e-7)
    return -(1/x-1).log()

# %% ../nbs/00_torch_core.ipynb #636e22ec
def num_distrib():
    "Return the number of processes in distributed training (if applicable)."
    return int(os.environ.get('WORLD_SIZE', 0))

# %% ../nbs/00_torch_core.ipynb #9737fbd2
def rank_distrib():
    "Return the distributed rank of this process (if applicable)."
    return int(os.environ.get('RANK', 0))

# %% ../nbs/00_torch_core.ipynb #df12b063
def distrib_barrier():
    "Place a synchronization barrier in distributed training"
    if num_distrib() > 1 and torch.distributed.is_initialized(): torch.distributed.barrier()

# %% ../nbs/00_torch_core.ipynb #564460b2
# Saving arrays requires pytables - optional dependency
try: import tables
except: pass

# %% ../nbs/00_torch_core.ipynb #551d57b7
def _comp_filter(lib='lz4',lvl=3): return tables.Filters(complib=f'blosc:{lib}', complevel=lvl)

# %% ../nbs/00_torch_core.ipynb #ce09bac5
@patch
...
# %% ../nbs/00_torch_core.ipynb #3c35ba6c
def doc(elt):
...
# %% ../nbs/00_torch_core.ipynb #c2a57272
...
# %% ../nbs/00_torch_core.ipynb #ed6ef8a3
...
# %% ../nbs/00_torch_core.ipynb #8dfe0195
...
# %% ../nbs/00_torch_core.ipynb #968f5cd7
...
# %% ../nbs/00_torch_core.ipynb #8c62d368
def requires_grad(m):
    "Check if the first parameter of `m` requires grad or not"
    ps = list(m.parameters())
    return ps[0].requires_grad if len(ps)>0 else False

# %% ../nbs/00_torch_core.ipynb #e29adcde
...
# %% ../nbs/00_torch_core.ipynb #f8131a12
def cond_init(m, func):
    "Apply `init_default` to `m` unless it's a batchnorm module"
    if (not isinstance(m, norm_types)) and requires_grad(m): init_default(m, func)

# %% ../nbs/00_torch_core.ipynb #abe3013b
def apply_leaf(m, f):
...
# %% ../nbs/00_torch_core.ipynb #f47bc086
def apply_init(m, func=nn.init.kaiming_normal_):
    "Initialize all non-batchnorm layers of `m` with `func`."
    apply_leaf(m, partial(cond_init, func=func))

# %% ../nbs/00_torch_core.ipynb #26b6915c
...
# %% ../nbs/00_torch_core.ipynb #0c96ea49
...
# %% ../nbs/00_torch_core.ipynb #d9853a0e
...
# %% ../nbs/00_torch_core.ipynb #8c128467
